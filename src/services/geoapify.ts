...
async function requestGeoapify<T>(url: string): Promise<T | null> {
...
interface GeoapifyPlacesResponse {
  features: Array<{
    properties: {
      ...
      lat: number;
      lon: number;
      datasource?: { raw?: { place_id?: string } };
    };
  }>;
}
...
interface GeoapifyAutocompleteResponse {
  results: Array<{
    ...
    lat: number;
    lon: number;
    rank?: { confidence_city_level?: string };
  }>;
}

export async function searchPlaces(query: string): Promise<PlaceSuggestion[]> {
  const trimmedQuery = query.trim();
  if (!trimmedQuery || !GEOAPIFY_API_KEY) return [];
  ...
  const data = await requestGeoapify<GeoapifyPlacesResponse>(
    `${GEOAPIFY_BASE_URL}/places?${params.toString()}`
  );

  return (
    data?.features?.map((item) => ({
      placeId: item.properties.place_id || item.properties.datasource?.raw?.place_id || '',
...
export async function getPlaceSuggestions(query: string): Promise<PlaceSuggestion[]> {
  const trimmedQuery = query.trim();
  if (!trimmedQuery || trimmedQuery.length < 3 || !GEOAPIFY_API_KEY) return [];
  ...
  const data = await requestGeoapify<GeoapifyAutocompleteResponse>(
    `${GEOAPIFY_BASE_URL}/geocode/autocomplete?${params.toString()}`
  );

  return (
    data?.results?.map((item) => ({
      placeId: item.place_id || item.rank?.confidence_city_level || item.formatted || '',
